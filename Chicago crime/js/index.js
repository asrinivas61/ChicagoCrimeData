...
lineReader.on('close', () => {
	var theft = fs.readFileSync('../output/theft.json').toString();
	var new_theft = theft.substring(0, theft.length-3);
	fs.writeFile('../output/theft.json', '');
	fs.appendFile('../output/theft.json', new_theft+'\n]');

	var assault = fs.readFileSync('../output/assault.json').toString();
	var new_assault = assault.substring(0, assault.length-3);
	fs.writeFile('../output/assault.json', '');
	fs.appendFile('../output/assault.json', new_assault+'\n]');
});